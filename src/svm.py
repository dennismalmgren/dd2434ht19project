...
class SVM(object):
	...
	def set_kernel(self, matrix):
		#Give the SVM a custom kernel
		self.kernel_matrix = matrix

	def kernel(self, indexa, indexb):
		return self.kernel_matrix[int(indexa), int(indexb)]
	...
	def classify_dataset(self,data):
		if self.solution_found:
			...
			return None
	def analyze(self):
		classifications = self.classify_dataset(self.dataset1)
		if type(classifications) != type(None):

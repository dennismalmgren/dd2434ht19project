...
Xs = np.random.multivariate_normal(
    [np.random.random() * meandistance,
     np.random.random() * meandistance], cov, (N / gaussians, ))
...
ys = np.array([-1] * N)
sidx = random.sample(np.where(ytrue == 0)[0],
...
model = CPLELearningModel(
    sklearn.svm.SVC(kernel=kernel, probability=True),

...
def evaluate_and_plot(model, Xs, ys, ytrue, lbl, subplot=None, block=False):
    ...
    plt.imshow(
        probas[:, 1].reshape((gridsize, gridsize)),
        extent=(minx, maxx, miny, maxy),
        origin='lower')

    # plot decision boundary
    try:
        if hasattr(model, 'predict_from_probabilities'
                   ) and model.predict_from_probabilities:
            plt.contour(
                (probas[:, 0] < np.average(probas[:, 0])).reshape((gridsize,
                                                                   gridsize)),
                extent=(minx, maxx, miny, maxy),
                origin='lower')
        else:
            plt.contour(
                model.predict(Xfull).reshape((gridsize, gridsize)),
                extent=(minx, maxx, miny, maxy),
                origin='lower')
    except:
    ...
    plt.scatter(
        ...
        s=(ys > -1) * 300 + 100,
        linewidth=1,
        edgecolor=[cols[p] * P[p] for p in model.predict(Xs).astype(int)],
        cmap='hot')
    plt.scatter(
        Xs[ys > -1, 0],
        Xs[ys > -1, 1],

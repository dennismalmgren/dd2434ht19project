...
class QN_S3VM:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator=None, **kw):
        ...
        if isinstance(X_l, csc.csc_matrix):
            ...
            self.__model = QN_S3VM_Sparse(X_l, L_l, X_u, random_generator,
                                          **kw)
...
class QN_S3VM_Dense:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        self.__size_l, self.__size_u, self.__size_n = len(X_l), len(
            X_u), len(X_l) + len(X_u)
    ...
    def mygetPreds(self, X, real_valued=False):
        ...
        KNU_bar_horizontal_sum = (1.0 / len(self.__X_u_subset)) * KNU_bar.sum(
            axis=1)
        KNR = KNR - KNU_bar_horizontal_sum - self.__KU_barR_vertical_sum + self.__KU_barU_bar_sum
        preds = KNR * self.__c[0:self.__dim - 1, :] + self.__c[self.__dim -
                                                               1, :]
        return preds
    ...
    def __setParameters(self, **kw):
        ...
        self.__lam_Uvec = [
            float(self.__lamU) * i
            for i in [0, 0.000001, 0.0001, 0.01, 0.1, 0.5, 1]
        ]
        ...
        self.__regressors_indices = sorted(
            self.__random_generator.sample(
                range(0, len(self.__X)), self.__numR))
    ...
    def __initializeMatrices(self):
        if self.__matrices_initialized == False:
            ...
            self.__Xreg = (mat(
                self.__X)[self.__regressors_indices, :].tolist())
            print('Compute kernel matrix __KLR...')
            self.__KLR = self.__kernel.computeKernelMatrix(
                self.__X_l, self.__Xreg, symmetric=False)
            print('Compute kernel matrix __KUR...')
            print(f'X_u shape: {len(self.__X_u)}')
            ...
            subset_unlabled_indices = sorted(
                self.__random_generator.sample(
                    range(0, len(self.__X_u)),
                    min(self.__max_unlabeled_subset_size, len(self.__X_u))))
            self.__X_u_subset = (mat(
                self.__X_u)[subset_unlabled_indices, :].tolist())
            print('Compute kernel matrix __KNU_bar...')
            ...
            print('Compute kernel matrix __KU_barR...')
            ...
            print('Compute kernel matrix __KU_barU_bar...')
    ...
    def __getFitness(self, c):
        ...
        preds_labeled_log_exp = multiply(preds_labeled_good_indicator,
        ...
        term2 = (float(self.__lamU) / float(self.__size_u)) * np.sum(
            np.exp(-self.__s * preds_unlabeled_squared))
    ...
    def __getFitness_Prime(self, c):
        ...
        preds_unlabeled_squared_exp_f = multiply(preds_unlabeled,
                                                 preds_unlabeled)
        preds_unlabeled_squared_exp_f = np.exp(
            -self.__s * preds_unlabeled_squared_exp_f)
        preds_unlabeled_squared_exp_f = multiply(preds_unlabeled_squared_exp_f,
                                                 preds_unlabeled)
    ...
    def __getTrainingPredictions(self, X, real_valued=False):
        ...
        print(f'training prediction: {preds.shape}')
        if real_valued == True:
            return preds.flatten(1).tolist()[0]
        else:
            return np.sign(np.sign(preds) + 0.1).flatten(1).tolist()[0]
...
class QN_S3VM_Sparse:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        if X_l.shape[1] > X_u.shape[1]:
            X_u = sparse.hstack([
                X_u,
                sparse.coo_matrix(X_u.shape[0], X_l.shape[1] - X_u.shape[1])
            ])
        elif X_l.shape[1] < X_u.shape[1]:
            X_l = sparse.hstack([
                X_l,
                sparse.coo_matrix(X_l.shape[0], X_u.shape[1] - X_u.shape[1])
            ])
        # We vertically stack the data matrices into one big matrix
        X = sparse.vstack([X_l, X_u])
        self.__size_l, self.__size_u, self.__size_n = X_l.shape[0], X_u.shape[
            0], X_l.shape[0] + X_u.shape[0]
        x = arr.array('i')
        for l in L_l:
            x.append(int(l))
        self.__YL = mat(x, dtype=np.float64)
    ...
    def __setParameters(self, **kw):
        ...
        if self.parameters['estimate_r'] != None:
            self.__estimate_r = float(self.parameters['estimate_r'])
        elif self.__YL.shape[
                0] > self.__minimum_labeled_patterns_for_estimate_r:
            self.__estimate_r = (1.0 / self.__YL.shape[0]) * np.sum(
                self.__YL[0:])
        else:
            self.__estimate_r = 0.0
    ...
    def __getFitness(self, c):
        ...
        preds_labeled = self.__surrogate_gamma * (
            1.0 - multiply(self.__YL,
                           (self.X_l * XTc - self.__mean_u * XTc) + b[0, 0]))
...
class RBFKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self._data1 = mat(data1)
        self._data2 = mat(data2)
        print(self._data1.shape, self._data2.shape)
        assert self._data1.shape[1] == (self._data2.T).shape[0]
        self._dim1 = len(data1)
        self._dim2 = len(data2)
        self._symmetric = symmetric
